...
public class ElementaryFunctions {
	...
	/**
	 * This function splits the number x into its normalized fraction f and exponent e, such that x = f * 2^e and 0.5 {@literal <}= f {@literal <} 1. The function returns f and stores the exponent in e. If x is zero, both f and e are set to zero. This function provides an alternative to the standard math function frexp(x, e).
	 *
	 * @param x double value
	 * @param n int value
	 * @return double[] value
	...
	/**
	 * This macro returns the sign of x. It is defined as ((x) {@literal >}= 0 ? 1 : -1). Note that with this definition the sign of zero is positive (regardless of its IEEE sign bit).
	 * @param x double value
	 * @return int value
	 */
	...
	/**
	 * This macro returns the maximum of a and b. It is defined as ((a) {@literal >} (b) ? (a):(b)).
	...
	/**
	 * This macro returns the minimum of a and b. It is defined as ((a) {@literal <} (b) ? (a):(b)).
	 * @param a double value
	 * @param b double value
	 * @return double value
	 */
	...
	/**
	 * This function determines whether x and y are approximately equal to a relative accuracy epsilon. The relative accuracy is measured using an interval of size 2 \delta, where \delta = 2^k \epsilon and k is the maximum base-2 exponent of x and y as computed by the function frexp(). If x and y lie within this interval, they are considered approximately equal and the function returns 0. Otherwise if x {@literal <} y, the function returns -1, or if x {@literal >} y, the function returns +1. Note that x and y are compared to relative accuracy, so this function is not suitable for testing whether a value is approximately zero.
	 * <br>
	 * The implementation is based on the package fcmp by T.C. Belding.
	 * @param x double value
	 * @param y double value
	 * @param epsilon double value
	 * @return int value
	 */
