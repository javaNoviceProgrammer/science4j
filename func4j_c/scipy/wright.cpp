...
const complex<double> I(0.0, 1.0); // this is j=(0,1)


int
wright::wrightomega_ext(complex<double> z, complex<double> *w, complex<double> *cond)
{
  ...
  /*********************************/
  else if(gsl_isinf(x) && (x < 0.0) && (-pi < y) && (y<= pi))
    {
      if (fabs(y) <= pi/2.0)
        {
	  if (y >= 0)
  ...
  /**************************/
  else if(gsl_isinf(x) || gsl_isinf(y))
    {
