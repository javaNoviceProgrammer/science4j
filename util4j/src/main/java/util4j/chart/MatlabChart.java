...
import util4j.array.ArrayUtils;
...
public class MatlabChart implements Serializable {
    ...
    public void plot(double[] x, Complex[] y) {
	    	double[] yReal = ArrayUtils.getReal(y) ;
	    	double[] yImag = ArrayUtils.getImag(y) ;
...
//    }
    ...
    public void run(boolean systemExit){
	    ...
	    chartFrame.add(cpanel) ;
	    chartFrame.setSize(640, 450);
        Image image = Toolkit.getDefaultToolkit().getImage(ClassLoader.getSystemResource("images/presentation.png"));
        chartFrame.setIconImage(image);
        chartFrame.setTitle("Plot Viewer v1.0");
        chartFrame.setVisible(true);
        if(systemExit){
    ...
    public ChartPanel getOriginalChartPanel(){
